...
/*
La funzione di libreria strlen() potrebbe essere (teoricamente) 
codificata in modo ricorsivo, come segue…
...
Ha lo stesso stesso effetto di strcpy() in string.h : 
s1 riceve tutti i  caratteri contenuti in s2 (incluso il '\0')
void strcpyRic( char *s1, char *s2 );

Come strcmp() in string.h : 
restituisce 0 se le stringhe sono uguali, n<0 (n>0) se
...
int strlenRic(const char * s) { //puntatore a un valore che non potra' essere cambiato dalla mia implementazione
	if (*s == '\0') return 0;
	else return 1 + strlenRic(s + 1);
}
