...
La lista rappresenta un indice di parole,
elencate in ordine alfabetico e
accompagnate da un numero, che rappresenta la pagina in cui
la parola compare
per la prima volta in un libro. La struttura della lista è:

*/
typedef struct Guscio {
	char * parola;
	int pagina; 
	struct Guscio * next;  } Nodo;
typedef Nodo* Indice;
...
int trovaPaginaNonRicorsiva(Indice idx, char * lemma) {
	// sono in fondo alla lista o ho superato la parola che cerco ritorno -1
	while (!(idx == NULL || strcmp(idx->parola, lemma) > 0)) {
		if (strcmp(idx->parola, lemma) == 0)
			return idx->pagina;
		else
			idx= idx->next
...
void   cleanup ( Indice * idx );   
oppure, a scelta, di prototipo
Indice cleanup ( Indice idx );

che elimina dall’indice idx (deallocandoli) tutti i nodi che 
contengono informazioni non valide 
(puntatori a NULL o stringhe di lunghezza zero come parole, 
valori negativi o nulli come numeri di pagina), 
lasciando gli altri nodi correttamente concatenati.
Nel caso del primo prototipo la testa della lista è passata 
per indirizzo,
nel caso del secondo la funzione restituisce un puntatore 
