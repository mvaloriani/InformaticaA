...
// (a)	Si codifichi in C la funzione ...hercom(...) che verifica la compatibilità
// Hertziana di due stringhe
...
int hertree(tree t) {
	...
	if (hertree(t->left) == 0) return 0;
	// valuto ricorsivamente l'albero di destra
	return hertree(t->right);
	//return hertree( t->right ) && hertree(t->left);
	//return hertree( t->right ) + hertree(t->left)==2 ? 1 : 0;
}
...
tree buildNode(char* word) {
	if (word != NULL) {
		...
		radice->word = word;
//		readice->left = radice->right = NULL;
		return radice;
	}
	return NULL;
}
