...
typedef struct Elem {
	int    concorrente; 
	Tempo  t_tappa;
	struct Elem * next; } Arrivo;
typedef Arrivo * Tappa;
typedef struct Nodo { 
	int concorrente; 
	Tempo t_totale;
	struct Nodo *next; } Posizione;
...
Si implementino, dopo averne definito precisamente i prototipi,
le funzioni
… sum(Tempo t1, Tempo t2, …); restituisce il tempo somma t = t1 + t2
… cmp(Tempo t1, Tempo t2, …); restituisce 0 se t1=t2, 
un intero positivo se t1>t2,
negativo altrimenti

Suggerimento:
si implementino e utilizzino le due funzioni
long intTempoCent(Tempo t) e
...
si progetti la funzione … aggiorna(…) che aggiorna la classifica
generale operando come segue:
...
agendo direttamente sulla lista originale 
(si perdono, quindi, i loro dati);
