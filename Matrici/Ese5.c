...
è diagonalmente dominante se la somma dei valori assoluti degli elementi
su ciascuna riga,escluso l'elemento sulla diagonale principale,
    ...
    for (i = 0; i < N && dom; i++) {
        ...
        for (j = 0; j < N; j++) {
            if (j == i) //sono sulla diagonale
                ...
                ele += abs(mat[i][j]); //ele = ele + abs(mat[i][j]);
        }
        if (diag <= ele)
            dom = 0;
    }
